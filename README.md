...
> A [Seneca.js][] neo4j store plugin example usage

# seneca-user-service-example
...
An example of how to use the [seneca-neo4j-store](https://github.com/DogFishProductions/seneca-neo4j-store) plugin to persist data using a graph database.  The standard Seneca [user plugin](https://github.com/senecajs/seneca-user) has been extended to accommodate a graph model and is started as a listening microservice.  The test client is a separate microservice that exists simply to exercise the extended user plugin.

### Support
If you have any questions regarding this example please feel free to contact me on Twitter: [@expert_in_it](http://twitter.com/expert_in_it)
### Seneca compatibility
Supports Seneca versions **1.x** and **2.x**

## Background
The [seneca-neo4j-store](https://github.com/DogFishProductions/seneca-neo4j-store) plugin works like any other Seneca [store plugin](https://github.com/senecajs/seneca/blob/master/doc/data-store.md). It allows us to persist [data entities](http://senecajs.org/tutorials/understanding-data-entities.html) without the need to know anything about how or where it is done.  This leads to a problem, however - if we use it as a simple replacement for, say, the MySQL plugin then all we've done is to exchange nodes in the graph database for rows in a SQL database.  We're not using the power of the graph in any way, so why bother?
...
![realm](https://github.com/DogFishProductions/seneca-user-service-example/raw/master/docs/graph1.png)
...
![realm](https://github.com/DogFishProductions/seneca-user-service-example/raw/master/docs/graph5.png)
...
### Running the service
I run all my code in Docker, and this example is no exception.  Neo4j, the user service and the test client will each run in their own container.

Start the neo4j container first: `docker-compose up neo4j`. Wait for this container to initialise itself then start the user container: `docker-compose up user`.  Once again, wait for this container to initialise itself and indicate that Seneca is listening and then start the test container to run the tests: `docker-compose up test`.

Enjoy!

## License
Copyright (c) 2016, Paul Nebel.
Licensed under [MIT](https://github.com/DogFishProductions/seneca-user-service-example/blob/master/LICENSE).

[travis-badge]: https://travis-ci.org/DogFishProductions/seneca-user-service-example.svg
[travis-url]: https://travis-ci.org/DogFishProductions/seneca-user-service-example
[david-badge]: https://david-dm.org/DogFishProductions/seneca-user-service-example.svg
[david-url]: https://david-dm.org/DogFishProductions/seneca-user-service-example
...
[github issue]: https://github.com/DogFishProductions/seneca-user-service-example/issues